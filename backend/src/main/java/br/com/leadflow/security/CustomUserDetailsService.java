...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        User user = userDAO.findByEmailIgnoreCase(username).orElseThrow(() -> new UsernameNotFoundException("Credenciais inválidas."));
        return new LeadFlowPrincipal(user);
    }

    @Transactional(readOnly = true)
    public LeadFlowPrincipal loadById(Long id) {
        User user = userDAO.findById(id).orElseThrow(() -> new UsernameNotFoundException("Usuário não encontrado."));
        return new LeadFlowPrincipal(user);
    }
}
