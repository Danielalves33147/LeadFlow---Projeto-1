...
public class AccessService {
    ...
    public User currentUser() {
        ...
        return userDAO.findById(id).orElseThrow(() -> new ResourceNotFoundException("Usuário autenticado não encontrado."));
    }

    public List<Long> authorizedBranchIds(User user) {
        if (user.getRole() == UserRole.ADMIN) {
            return branchDAO.findByCompanyIdOrderByNameAsc(user.getCompany().getId()).stream().map(Branch::getId).toList();
        }
        if (user.getRole() == UserRole.MANAGER) {
            List<Long> ids = userBranchDAO.findByUserId(user.getId()).stream().map(ub -> ub.getBranch().getId()).distinct().toList();
            if (!ids.isEmpty()) return ids;
        }
        return user.getPrimaryBranch() == null ? List.of() : List.of(user.getPrimaryBranch().getId());
    }

    public Branch requireBranch(Long branchId, User actor) {
        Branch branch = branchDAO.findByIdAndCompanyId(branchId, actor.getCompany().getId())
            .orElseThrow(() -> new ResourceNotFoundException("Filial não encontrada."));
        if (actor.getRole() != UserRole.ADMIN && !authorizedBranchIds(actor).contains(branchId)) {
    ...
    public User requireCompanyUser(Long userId, User actor) {
        return userDAO.findByIdAndCompanyId(userId, actor.getCompany().getId())
            .orElseThrow(() -> new ResourceNotFoundException("Usuário não encontrado."));
    }

    public void assertLeadAccess(Lead lead, User actor) {
        if (!lead.getBranch().getCompany().getId().equals(actor.getCompany().getId())) throw new AccessDeniedBusinessException("Lead fora do seu contexto.");
        if (actor.getRole() == UserRole.ADMIN) return;
        if (actor.getRole() == UserRole.MANAGER && authorizedBranchIds(actor).contains(lead.getBranch().getId())) return;
        if (actor.getRole() == UserRole.SELLER && lead.getResponsibleUser().getId().equals(actor.getId())) return;
