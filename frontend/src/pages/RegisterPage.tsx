...
function strength(value: string) {
  ...
  return score < 3 ? 'Fraca' : score < 5 ? 'Boa' : 'Forte';
}
    ...
    <main className="lf-auth-form-panel">
      <form className="lf-auth-form register" onSubmit={submit}>
        ...
        <div className="lf-form-grid">
          ...
          <Field label="Senha" error={fields.password} helper={`Força: ${strength(form.password)}. Use 8+ caracteres com letras, número e símbolo.`}><Input required minLength={8} type="password" value={form.password} onChange={(e) => set('password', e.target.value)} /></Field>
          <Field label="Confirmação da senha" error={fields.confirm}><Input required minLength={8} type="password" value={form.confirm} onChange={(e) => set('confirm', e.target.value)} /></Field>
        </div>
